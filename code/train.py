...
checkpoint = ModelCheckpoint('weights_resA.h5',save_weights_only = True, period = 1)
checkpointB = ModelCheckpoint('weights_resB.h5', save_weights_only= True, period = 1)
model.load_weights('weights_resA.h5')
history=model.fit_generator(train_generator,
                   steps_per_epoch = 1800,
                   epochs = 30, validation_data = val_generator,callbacks=[checkpoint])
...
historyB=new_model.fit_generator(small_train_generator,
                   steps_per_epoch =5,
                   epochs = 12, validation_data = small_val_generator,callbacks=[checkpointB])
